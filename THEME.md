...
## 7. Ẩn / hiện bài (tour & tin tức)

Admin có thể **ẩn một bài khỏi website** mà không xoá — bài vẫn nằm trong admin để bật lại.

- **Cách dùng**: vào `/admin/tours` hoặc `/admin/news`
  - bấm nút **Ẩn / Hiện** ngay trong danh sách (nhanh), hoặc
  - mở form sửa → tick **“Ẩn khỏi website”** ở cuối form.
...
- **Ẩn có tác dụng gì**: `isVisible()` trong `site/src/lib/content.js` lọc ở
  ...
  sitemap, và **trang chi tiết cũng không được build** (404).
- ⚠️ Worker phải được deploy lại (có `hidden` trong `dev/worker/src/repo/content.ts`)
  thì cờ mới được ghi khi lưu từ admin.
